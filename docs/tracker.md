...
Last updated: 2026-09-26, by the orchestrator, after validating the pre-M6
fix round (#74–#76). `main` is green at b8cf9b0.
...
- **Full-system verification is done** (#72,
  ...
  against 301s).
- **The pre-M6 fix round is done** — #74 (resolver and unreachable-host
  classification), #75 (the OAuth callback's 500s, the C-7 address leak,
  OpenAPI statuses), #76 (the tracker's remaining fix rows). Every head was
  confirmed by Codex after its last push. A DNS `SERVFAIL` now stores
  `down / dns_failure / ESERVFAIL` where the verification run stored
  `unknown / unknown_error`, so M6's incidents can see a DNS outage. A 40s
  database outage answers `503` on every route, never `500`, and the worker
  logs 6 lines per site instead of about 40. The `updated_at` flake had two
  measured causes — the Docker VM's clock 2.5–3.5ms ahead of the host, and
  millisecond parsing of a microsecond column — and went from 9 failures in 25
  to none.
- **Defect 5 is decided and fixed in #78**: the code follows PRD §6.5 —
  timeout under interval on create and on PATCH under the row lock, thresholds
  1–10, the PRD's six methods — and migration 0010 repairs stored rows, then
  adds `CHECK` constraints. Once #78 merges, nothing stands between `main` and
  M6.
...
| #75            | `oauth.start()` writes its pending row before provider discovery, so a failed discovery leaves an orphan row until the TTL sweep removes it. Harmless; noted so nobody rediscovers it.                                                                                                                                                                                                                                                                                                                                 | decide  |
| #74            | With the SSRF guard off, `getaddrinfo`'s `EAI_FAIL` has no row and would store `unknown_error`. No run has reached it; a one-line fix when something does.                                                                                                                                                                                                                                                                                                                                                             | fix     |
| #76            | `service.repository.int.test.ts` failed once (2 tests) and passed on two reruns, with the failure text not captured. Same signature as the machine-stall row; capture the output next time before calling it a flake.                                                                                                                                                                                                                                                                                                  | process |
| #75 process    | #75's branch was first pushed with `--no-verify`, before its review receipt, to get something to clone from. No PR was open, so nothing unreviewed reached Codex, but it is the gate being skipped for convenience. A fresh clone can come from the local repository — the worker switched to that — and `CLAUDE.md` should say so.                                                                                                                                                                                    | process |
| #78            | PRD §6.5's threshold bound now lives twice: `INCIDENT_THRESHOLD_MAX = 10` in TypeScript and `BETWEEN 1 AND 10` in migration 0010's `CHECK`. Right for a specification constant, since changing it should take a migration, but nothing ties the two together. A test comparing the constant against `pg_constraint` would.                                                                                                                                                                                             | fix     |
