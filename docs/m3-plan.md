...
| D69 | A write that misses its deadline also tears the stalled stream down, with `stream.destroy()` and no error argument                                                                                                                                                                                                                                                                                                            | Rejecting the wrapper promise and leaving the write pending (D66)                                                              | The rejection frees the transaction, not the stream: `stream.write()` stays pending, and a pending write keeps Node's event loop alive. The removal rolls back and releases its row lock, and then the mandatory audit hangs instead of exiting — and since D68 sets `process.exitCode` rather than calling `process.exit()`, nothing forcibly ends it any more, so D68 is what made this reachable. No error argument, deliberately: `destroy(err)` makes the stream emit `error`, and the final diagnostic runs after the CLI has removed its scoped listeners, so that event would become the uncaught exception D65 exists to prevent (Codex finding, PR #43, P2)                                                                                                         |
...
| A timed-out write does not keep the process alive (D69)                                             | `stream-write.test.ts`: a fake stalled stream asserts `destroy()` is called exactly once on expiry and the promise still rejects; a stream whose `destroy()` throws still reports the timeout rather than the teardown failure; and a healthy stream is never destroyed. Stated plainly: the teardown is proved, the resulting process exit is not — forcing a real stalled pipe means filling the OS buffer, and that timing dependence is what CLAUDE.md rules out as evidence; removal: drop the destroy call and the first case fails on a destroy count of 0                                                                                                                                                     |
