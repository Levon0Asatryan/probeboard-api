...
overwrite the already-decided outcome. Tested directly (§7): a scenario where
...
- `json_path`: checks the truncation flag (§3.6) **first**, same as
  ...
  **D36/D42 stop new rows from getting an unsupported path — they do
  ...
  - **Reversibility (D51 — Codex finding, PR #40):** it needs no
    ...
    redeploy.
  - **When:** once, after PR1's schema constraint is deployed and before
    M4's scheduler starts probing — stated in §8's PR1 delivery and §9's
...
accepting any non-empty string, plus the OpenAPI `pattern` taken from that
module's exported `JSON_PATH_PATTERN` (D49/D52) and a document-drift test
proving it, the `audit-json-path-assertions` maintenance script and its
`npm run` entry (D48/D50/D51 — run once after deploy, before M4 probes;
not a migration), and `http/registration.http` updated with a
...
request, same rule as every other endpoint, CLAUDE.md's own "a collection
that lags the code is worse than none"). Unit-tested
