...
- `json_path`: checks the truncation flag (§3.6) **first**, same as
  ...
  **D36/D42 stop new rows from getting an unsupported path — they do
  ...
  - **The candidate scan is paged, not one unbounded `SELECT` (D59 — Codex
    ...
    endpoint quota permits 100,000 per user and nothing bounds a deployment,
...
| D59 | The candidate scan pages through `endpoints` by keyset on `id` (`ORDER BY id`, `LIMIT`, `WHERE id > last`), repairing each page before fetching the next                                                                                                                                                                                                                                                                      | One unbounded `SELECT id, assertions FROM endpoints`                                                                           | The unbounded read materializes every endpoint's full assertions array in the maintenance process before a single row is repaired, clean rows included; the quota allows 100,000 endpoints per user with no deployment-wide bound, so the audit D48 makes mandatory before M4 probes is the run that exhausts memory or hands PostgreSQL one enormous read. Keyset, not `OFFSET`: the scan runs while rows are rewritten and `OFFSET` re-counts from the start each page (Codex finding, PR #43, P2)                           |
