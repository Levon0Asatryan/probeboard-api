...
- `json_path`: checks the truncation flag (§3.6) **first**, same as
  ...
  **D36/D42 stop new rows from getting an unsupported path — they do
  ...
  - **Nothing is lost silently:** the script prints each
    `{endpointId, removedAssertion}` as JSON on stdout **inside that row's
...
| D51 | No `.down.sql` is specified for the audit, because it is not a migration; the printed `{endpointId, removedAssertion}` record is the reversal path, and re-running is a no-op                                                                                                                                                                                                                                                 | A paired up/down migration for a data-only cleanup                                                                             | Nothing under `src/core/db/migrations/` changes, so the migrator's paired-file rule and CI `migrations` job are untouched; a removed assertion is recoverable from the script's own output through the normal API (Codex finding, PR #40)                                                                                                                                                                                                                                                                                      |
