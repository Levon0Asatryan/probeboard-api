...
| A broken stdout rolls the removal back instead of killing the process (D65)                         | Real broken pipe against the compose database: seed an unsupported assertion and run the CLI with stdout closed early (`                                                                                                                                                                                                                                                                                                                       | head -0`); asserts the process reports the failure rather than dying on an unhandled `error`event, and`psql` shows the row still carrying its assertion because the transaction rolled back; removal: drop the listeners and watch the same run die with "Unhandled 'error' event" before the rollback can run |
