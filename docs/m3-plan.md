...
- `json_path`: checks the truncation flag (§3.6) **first**, same as
  ...
  **D36/D42 stop new rows from getting an unsupported path — they do
  ...
  - **Nothing is lost silently:** the script prints each
    `{endpointId, removedAssertion}` as JSON on stdout **as that row's own
    transaction commits, not after the whole scan returns (D57 — Codex
    finding, PR #43, P1).** Because each removal is durable the moment its
    per-row transaction commits (D54), a record printed only at the end does
    not exist yet for any row already rewritten: a database error, a
    `SIGTERM`, or a failing stdout partway through would leave those
    assertions deleted with nothing to reconstruct them from — destroying
    exactly the reversal path D51 offers in place of a `.down.sql`. A throw
    from the print sink therefore aborts the run rather than continuing: if
    the record cannot be written down, removing further assertions would
    destroy more that also could not be recorded. That output is the
