...
CREATE TABLE probe_results (
    ...
    interval_s      integer       NOT NULL,   -- spacing to the NEXT slot: the span this probe represents
...
- `planWindow(from, to, now, retention)` — tiles `[from, to)` with `d1` for
  ...
  answer would include or omit part of a boundary minute. Availability is
  taken from what **actually exists**, not from config: the caller passes
  `retainedFrom` per grain, read by `StatsRepository` as the oldest live
...
| D7  | Buckets are anchored on `started_at` in UTC; `interval_s` is the interval the claim used to space the **next** slot — the span this probe represents — from the claim's own `scheduled_interval_s`, post-update (the statement writes `next_run_at` and `scheduled_interval_s` from the same `e.interval_s`) | time-weighting must use the interval the row represents (03 §3.5.1 case 1), not the endpoint's current one                                                                                                                                                                                                                                  |
...
| D17 | Window planner rejects a window whose edges need a grain already retired, judged by the oldest partition that exists (`retainedFrom`), not by config                                                                                                                                                         | rounding a window silently changes a number a user trusts                                                                                                                                                                                                                                                                                   |
