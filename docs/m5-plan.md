...
```sql
CREATE TYPE probe_outcome AS ENUM ('up', 'down', 'degraded', 'unknown');
CREATE TYPE failure_class AS ENUM ( /* the 16 FailureClass values, verbatim */ );
...
`failure_class` and `failure_code` are persisted on **every** non-success row,
...
3. Loop until `last_xid = horizon`: pick the batch's exclusive upper bound
   `upper` — the `ROLLUP_BATCH_ROWS`-th distinct `insert_xid` at or above
   `last_xid`, or `horizon` if fewer remain — then fold every row with
   `last_xid ≤ insert_xid < upper` (§3.5) and set `last_xid := upper`. Bounds
   are whole xids, so a transaction's rows are never split across batches, and
   no xid arithmetic is needed (`xid8` has none, §2.4.8). An idle system
   reaches `horizon` in one empty pass and does not re-scan the range.
...
- **Stats partitions are never older than the raw they serve:** a `refine`
  requires `RETENTION_M1_DAYS ≥ RETENTION_RAW_DAYS` and
  `RETENTION_H1_DAYS ≥ RETENTION_RAW_DAYS`. Otherwise a raw row still awaiting
...
3. `SET lock_timeout = MAINTENANCE_LOCK_TIMEOUT_MS`, then, as a **single
...
4. `DROP TABLE p`. A detached leftover from a crash between 3 and 4 — no
...
- `planWindow(from, to)` — tiles `[from, to)` with `d1` for whole UTC days and
  `h1` for whole hours at the edges. A window that reaches beyond the m1
  horizon must be **hour-aligned**; an unaligned one is rejected with a typed
  error rather than silently rounded. (M8 decides how the UI aligns.)
...
| D15 | The guard: a raw partition with unfolded rows is never dropped; stats retention ≥ raw retention (`refine`)                                                                                        | unrecoverable loss, and a poisoned fold, are the two ways retention can break the rollup                                                                                                                                                                                                                                                    |
| D16 | `le` buckets; interpolate inside the bucket, clamped to the observed min/max                                                                                                                      | matches Prometheus; §3.8                                                                                                                                                                                                                                                                                                                    |
| D17 | Window planner rejects an unaligned window past the m1 horizon                                                                                                                                    | rounding a window silently changes a number a user trusts                                                                                                                                                                                                                                                                                   |
...
| D21 | `failure_class` is an enum; a drift test asserts the TS union equals the DB labels, and `probe_outcome` likewise                                                                                  | a new `FailureClass` without a migration would fail every insert of that class — a failure that is itself one of the two measurement defects `AGENTS.md` names                                                                                                                                                                              |
...
equals percentile-of-concatenation); `planWindow` (aligned, unaligned inside and
